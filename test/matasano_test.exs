defmodule MatasanoTest do
  ...
  test "set 1 challenge 1 - convert hex to base64" do
    ...
    assert Matasano.Helper.hex_to_base64(hex) == base64
  end
  ...
  test "set 1 challenge 2 - fixed xor" do
    ...
    assert Matasano.Helper.fixed_xor(a, b) == output
  end
  ...
  test "set 1 challenge 3 - single-byte xor cipher" do
    ...
    assert Matasano.XorCipher.decrypt_single_byte_xor(ciphertext) == plaintext
  end
  ...
  test "set 1 challenge 4 - detect single-character xor" do
    ...
    assert Matasano.XorCipher.detect_single_byte_xor(data) == plaintext
  end
  ...
  test "set 1 challenge 5 - implement repeating-key xor" do
    ...
    assert Matasano.Helper.repeating_xor(key, message) == output
  end
  ...
  test "set 1 challenge 6 - break repeating-key xor" do
    ...
    assert Matasano.XorCipher.break_repeating_key_xor(data) == key
  end
  ...
  test "set 1 challenge 7 - aes in ecb mode" do
    ...
    assert Matasano.AES.decrypt_aes_128_ecb(data, key) == plaintext
  end
  ...
  test "set 1 challenge 8 - detect aes in ecb mode" do
    ...
    assert Matasano.AES.detect_aes_in_ecb(data, 32) == output
  end
  ...
  test "set 2 challenge 10 - implement cbc mode" do
    data = Matasano.IO.bytes_from_base64(Path.join("data", "10.txt"))
    key = "YELLOW SUBMARINE"
    iv = List.duplicate(0, 16) |> IO.iodata_to_binary
    plaintext = File.read!(Path.join("data", "play-that-funky-music.txt"))
